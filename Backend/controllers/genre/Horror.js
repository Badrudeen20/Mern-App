...
module.exports = {
    genre:async function(req,res){
        ...
        const list = await Movie.find({genres:/Horror/}).limit(parseInt(req.query.limit)).skip(startindex).exec()  
        const limit = await Movie.find({genres:/Horror/}).countDocuments()
        try{
